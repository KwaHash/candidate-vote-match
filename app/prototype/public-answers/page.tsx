 ...
 *       特定候補を有利/不利に扱わず、全候補者同一条件。提出は運営確認を経て公開。
 */
...
const ME = '区民 一郎' // ログイン中の候補者（プロト）
const ELECTION = '杉並区長選挙'

interface SentQuestion { id: string; category: string; title: string; body: string; votes: number; deadline: string }
const SENT_QUESTIONS: SentQuestion[] = [
  { id: 'q-seed-1', category: '財政・税金', title: '杉並区の財政をどう評価し、どう改善しますか？', body: '今後4年間で、どの支出を増やし、どの支出を見直すか具体的に教えてください。', votes: 142, deadline: '2026-07-02' },
  { id: 'q-seed-2', category: '防災・安全', title: '災害時の死者を減らすため、最初の1年で何を優先しますか？', body: '避難所環境・高齢者避難・要支援者支援の優先順位を教えてください。', votes: 98, deadline: '2026-07-02' },
  { id: 'q-seed-3', category: '子育て・教育', title: '最初の1年で実行する子育て政策は何ですか？', body: '財源とあわせて、最優先の施策を教えてください。', votes: 76, deadline: '2026-07-05' },
]

type AnsState = '未回答' | '下書き' | '提出済み（運営確認中）'
interface AnswerData { text: string; sourceUrl: string; date: string; status: AnsState }
...
export default function PublicAnswersPage() {
  ...
  const get = (id: string): AnswerData => answers[id] ?? { text: '', sourceUrl: '', date: '', status: '未回答' }
  ...
  const submit = (id: string) => { const a = get(id); if (!a.text.trim()) { flash('回答本文を入力してください'); return } update(id, { status: '提出済み（運営確認中）', date: today() }); flash('回答を提出しました（運営確認後に国民側へ公開）') }
  const flash = (m: string) => { setSavedMsg(m); setTimeout(() => setSavedMsg((x) => (x === m ? null : x)), 2500) }

  const answeredCount = SENT_QUESTIONS.filter((q) => get(q.id).status === '提出済み（運営確認中）').length
  const statusCls = (s: AnsState) => s === '提出済み（運営確認中）' ? 'bg-emerald-50 text-emerald-700' : s === '下書き' ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-500'
  ...
  return (
    <div className='mx-auto w-full max-w-2xl px-4 py-8'>
      <div className='mb-4'>
        ...
        <p className='mt-1 text-sm text-gray-500'>{ELECTION}・{ME} 様。有権者の<strong className='text-gray-700'>公開質問ボード</strong>で投票上位となった質問が、運営から<strong className='text-gray-700'>全候補者へ同一条件</strong>で届いています。</p>
      </div>
        ...
        {SENT_QUESTIONS.map((q) => {
          ...
          return (
            <div key={q.id} className='rounded-xl border border-gray-200 bg-white p-4'>
              ...
              <textarea value={a.text} onChange={(e) => update(q.id, { text: e.target.value })} placeholder='回答本文（中立・具体的に。財源や期限・KPIも書くと有権者に伝わります）' className='mt-3 h-24 w-full rounded-lg border border-gray-300 p-2 text-sm focus:border-rose-400 focus:outline-none' />
              ...
              <div className='mt-2 flex flex-wrap items-center gap-2'>
                ...
                {a.date && <span className='text-[10px] text-gray-400'>回答日 {a.date}</span>}
              </div>
            </div>
          )
        })}

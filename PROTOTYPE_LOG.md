...
## MVP再設計 Phase1（2026-06-20）— 機能を絞る
- 一覧（`app/prototype/page.tsx`）を **5メニュー**に再編: ①今日やること ②政策をつくる ③発信をつくる ④支援者を管理する ⑤資金・成果を報告する（＋サブ: プロフィール/公開LP）。
- 統合: AI政策室→AI政策秘書 / 選挙活動タスク→ダッシュボード / 支援募集→支援者CRM / LINE・SNS→発信をつくる。
- 名称変更: クラウドファンディング作成 →「政策応援ページ作成」（法務注意文を追加）。
- 後回し（初期非表示セクションへ隔離・コードは残置）: 候補者AIアバター / 自動化センター / AIツール連携 / 政策実装スコア / 選挙活動タスク（単体）。
- コンプライアンス注意・「AIは下書き→確認→公開（勝手に公開しない）」を明記。
- 次: Phase2 政策・1枚政策カード → Phase3 CRM強化 → Phase4 応援ページ → Phase5 公開ページ。

## 国民側・運営admin連携（2026-06）— 政治家側
